...
def main():
    ...
    for size in sorted(all_data.keys(), key=lambda x: int(x.split("x")[0])):
        ...
        mean, std, (std_low, std_high), (ci_low, ci_high), shap_p, dag_p = (
            compute_stats(gflops)
        )
        std_str = f"{std:.0f} ({std_low:.0f}–{std_high:.0f})"
        ci_str = f"{ci_low:.0f}–{ci_high:.0f}"
        print(
            f"{size:<12} {mean:8.0f} {std_str:>20} {ci_str:>20} {shap_p:12.4f} {dag_p:14.4f}"
        )
        rows.append(
            [size, mean, std, std_low, std_high, ci_low, ci_high, shap_p, dag_p]
        )
